 ...
 *  Tweaked again by: Jeremy Huckeba
 */
definition(
    name: "Double Tap",
...
preferences {
	...
	section("And dim lamps all the way when turning on") {
		input "isFullDim", "bool", required: false
	}
}
...
def switchHandler(evt) {
	...
	if (evt.physical) {
		if (evt.value == "on" && lastTwoStatesWere("on", recentStates, evt)) {
            ...
            if(!isFullDim) {
            	// set onSwitches to previous level
				onSwitches()*.on()
            } else {
            	// set onSwitches to 100% if they support dimming
                onSwitches().each { onSwitch -> 
                	if(onSwitch.hasCommand("setLevel")) {
                    	onSwitch.setLevel(100)
                    } else {
                    	onSwitch.on()
                    }
                }
