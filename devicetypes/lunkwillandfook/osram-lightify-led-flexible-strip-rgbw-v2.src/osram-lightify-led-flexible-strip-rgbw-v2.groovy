/* 
    Osram Flex RGBW Light Strip v2
...
metadata {
	definition (name: "OSRAM LIGHTIFY LED Flexible Strip RGBW v2", namespace: "LunkwillAndFook", author: "SmartThings") {
	...
	tiles {
        ...
        valueTile("colorTemp", "device.colorTemperature", inactiveLabel: false, width: 2, height: 2) {
			state "colorTemperature", label: '${currentValue} K'
		}
        valueTile("colorName", "device.colorName", inactiveLabel: false, width: 4, height: 2) {
	...
	if (description?.startsWith("catchall:")) {
        ...
        if (descMap.cluster == "0300") {
            if(descMap.attrId == "0007"){
                ...
                sendEvent(name: "colorName", value: "--", displayed:false)
...
def setColorTemperature(value, transitionTimeSeconds) {
    log.trace "setColorTemperature($value, $transitionTimeSeconds)"
    state?.colorType = "white"
	if(value<101){
...
def setLevel(def value, def transitionTimeSeconds) {
	...
	cmds << zigbeeSetLevel(level, transitionTimeSeconds)
...
def setColor(value, transitionTimeSeconds) {
    ...
    if (value.level) {
        ...
        cmd << zigbeeSetLevel(level, transitionTimeSeconds)
    }
...
//    cmds << "delay 150"
//    cmds << zigbeeSetLevel(maxLevel, 1)
//    cmds << "delay 25"
//    cmds << "st cmd 0x${device.deviceNetworkId} ${endpointId} 6 0 {}"
//    cmds << zigbeeSetLevel(maxLevel, 1)
//    cmds << "delay 100"
//    cmds << setColor(lightningBlueValue, 1000)
    //cmds << "delay 150"
    //cmds << "st cmd 0x${device.deviceNetworkId} ${endpointId} 0x300 0x00 {a7 00 1500}, delay 150, st cmd 0x0x${device.deviceNetworkId} ${endpointId} 0x300 0x03 {fe 1500}"
	//cmds << "st cmd 0x${device.deviceNetworkId} ${endpointId} 6 1 {}"
	//cmds << "delay 200"
	//cmds << "st cmd 0x${device.deviceNetworkId} ${endpointId} 0x300 0x00 {${scaledBlueHueValue} 00}"
	//cmds << "delay 200"
	//cmds << "st cmd 0x${device.deviceNetworkId} ${endpointId} 0x300 0x03 {${scaledBlueSatValue}}"
    //cmds << "delay 200"
    //cmds << zigbeeSetLevel(maxLevel, 1)
